...
export class OrderData {
  ...
  get isValid(): boolean | string {
    ...
    if (this.transport > 1 && this.name === undefined)
      return 'entregas precisa de um resposavel para entrega';
    if (this.transport > 2 && this.name!.trim().length === 0)
      return 'nome de quem ira fazer a entrega não pode ser vazio';
    if (this.transport > 3 && this.port === undefined)
      return 'entregas em porto precisam ter o nome do porto definido';
